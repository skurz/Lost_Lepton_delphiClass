...
void CompareControlRegions(string option="", int pull=0){ // string option="QCD"
  ...
  TDirectory *dPre = 0;
  TDirectory *dExp = (TDirectory*)LLFile->Get("Prediction_MC");
  ...
  TH1D* hEXTRAP_SF;
  if (doDataVsMC){
    hEXTRAP_SF = (TH1D*)dPre->Get("EXTRAP_weight_SF_LL");
  } else {
    hEXTRAP_SF = (TH1D*)dPre->Get("EXTRAP_weight_SF_LL_MC");
  }
  
  if(doDataVsMC){
    EstHistTemp=(TH1D*) dPre->Get("totalCS_LL")->Clone();
    EstHistDTemp=(TH1D*) dPre->Get("totalCS_LL")->Clone();
  }else{
    EstHistTemp=(TH1D*) dPre->Get("totalCS_LL_MC")->Clone();
    EstHistDTemp=(TH1D*) dPre->Get("totalCS_LL_MC")->Clone();
  }

  GenHistTemp=(TH1D*) dExp->Get("totalCS_LL_MC")->Clone();
  GenHistDTemp=(TH1D*) dExp->Get("totalCS_LL_MC")->Clone();

  for (int ibin(0); ibin<72; ibin++) {
    GenHistTemp->SetBinContent(ibin+1, GenHistTemp->GetBinContent(ibin+1)/hEXTRAP_SF->GetBinContent(ibin+1));
    GenHistDTemp->SetBinContent(ibin+1, GenHistDTemp->GetBinContent(ibin+1)/hEXTRAP_SF->GetBinContent(ibin+1));
    GenHistTemp->SetBinError(ibin+1, GenHistTemp->GetBinError(ibin+1)/hEXTRAP_SF->GetBinContent(ibin+1));
    GenHistDTemp->SetBinError(ibin+1, GenHistDTemp->GetBinError(ibin+1)/hEXTRAP_SF->GetBinContent(ibin+1));
  }
  ...
  sprintf(ytitlename,"Events in CR");
  gPad->SetLogy();
  ...
  // Legend & texts
  sprintf(tempname,"Lost-lepton control region");
  catLeg1->SetHeader(tempname);
  //sprintf(tempname,"#tau_{hadronic} BG expectation (MC truth)");
  sprintf(tempname,"#Delta#theta_{T} extrapolation");
  ...
  else sprintf(tempname,"Classical");
  ...
  sprintf(tempname,"jack_macros/plots/MC_CR_Plot_EXTRAP.pdf");
